...
# ─────────────────────────────────────────────────────────────
MAX_RPM = 15        # Paid tier default
MAX_TPM = 1_000_000 # Paid tier default
MAX_RPD = 1_500     # Daily limit (check AI Studio)
    ...
    async def count_tokens(self, contents: List[Dict], key: str, key_state=None) -> int:
        try:
            ...
            if r.status_code == 200:
                ...
                return count
            if r.status_code == 403 and key_state:
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        await supabase.save_message(user_id, "assistant", result["reply"], thought_signature=None)
