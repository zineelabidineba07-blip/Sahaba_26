...
# Logging Setup — Official FastAPI recommendation
logging.basicConfig(
    level=logging.INFO,
...
# Silence HTTP/2 internals noise — httpx official docs
...
# ─── Model Configuration — Official Google AI Docs
# Source: https://ai.google.dev/gemini-api/docs/models# NOTE: Ensure 'gemini-3-flash-preview' is enabled in your quota.
# If not, switch to 'gemini-1.5-flash' or 'gemini-2.0-flash-exp'.
MODEL_NAME   = "gemini-3-flash-preview"
GEMINI_BASE  = "https://generativelanguage.googleapis.com/v1beta/models"
TELEGRAM_API = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"

# ─────────────────────────────────────────────────────────────
# RATE LIMITS — Conservative defaults (adjust via AI Studio)
...
# ─────────────────────────────────────────────────────────────
# KEY STATE TRACKING — Minimal, documented pattern
# ─────────────────────────────────────────────────────────────
@dataclass
class KeyMetrics:
    rpm: int = 0
...
class KeyState:
    ...
    def can_accept(self, estimated_tokens: int) -> bool:
        ...
        if self.metrics.rpd >= SAFE_RPD:
            return False
        if (self.metrics.tpm + estimated_tokens) > SAFE_TPM:
            return False
        return True
...
# ─────────────────────────────────────────────────────────────
# SMART KEY ORCHESTRATOR — Documented load-balancing pattern
# ─────────────────────────────────────────────────────────────
class SmartKeyOrchestrator:
    ...
    async def get_best_key(self, estimated_tokens: int) -> Optional[KeyState]:
        async with self.lock:
            ...
            if not available:
                return None
            # Simple round-robin for documented reliability            best = min(available, key=lambda k: k.metrics.total_requests)
            return best
...
# ─────────────────────────────────────────────────────────────
# SUPABASE CLIENT — Official REST API pattern
# Source: https://supabase.com/docs/reference/python/rest-api
# ─────────────────────────────────────────────────────────────
class SupabaseClient:
    ...
    async def get_history(self, user_id: str, limit: int = 20) -> List[Dict]:
        try:
            r = await self.client.get(                f"{SUPABASE_URL}/rest/v1/messages",
...
# - https://ai.google.dev/gemini-api/docs/authentication
# ─────────────────────────────────────────────────────────────
class GeminiClient:
    ...
    def _make_headers(self, key: str) -> Dict:
        # Official: x-goog-api-key header
        return {
    ...
    def _estimate_tokens(self, contents: List[Dict]) -> int:
        ...
        return total + 100  # system instruction overhead
    ...
    async def count_tokens(self, contents: List[Dict], key: str) -> int:
        # Official: countTokens endpoint includes systemInstruction
        try:
            payload = {
                "contents": contents,
    ...
    async def generate_response(self, messages: List[Dict]) -> Dict:
        ...
        # Get key with simple round-robin
        ks = await self.orchestrator.get_best_key(1000)        if not ks:
            raise HTTPException(503, "No keys available")

        # Accurate token count via official API
        total_input = await self.count_tokens(contents, ks.key)
        max_output = min(1024, SAFE_TPM - total_input - 200)
        max_output = max(256, max_output)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if r.status_code == 429:
                    ...
                    ks = await self.orchestrator.get_best_key(1000)
                    if not ks:
...
# ─────────────────────────────────────────────────────────────
# TELEGRAM CLIENT — Official Bot API
...
class TelegramClient:
    ...
    async def set_webhook(self, url: str) -> bool:
        ...
        try:
            ...
            data = r.json()
            if data.get("ok"):
                logger.info(f"✅ Webhook set → {url}")                return True
...
# ─────────────────────────────────────────────────────────────
# FASTAPI APP — Official lifespan pattern
...
async def lifespan(app: FastAPI):
    ...
    await http_client.aclose()    logger.info("👋 Bot shutdown")

app = FastAPI(
    title="سحابة | Sahaba Bot",
...
# ─────────────────────────────────────────────────────────────
# TELEGRAM WEBHOOK HANDLER — Official pattern
# ─────────────────────────────────────────────────────────────
@app.post("/webhook")
...
# ─────────────────────────────────────────────────────────────
# HEALTH & STATUS ENDPOINTS — Official Render pattern
...
async def root(request: Request):
    ...
    return {
        ...
        "features": {
            ...
            "countTokens_api": "✅ accurate token counting",
            "x-goog-api-key": "✅ official auth header",
            "http2": "✅ performance optimized",
            "smart_keys": f"✅ {len(GEMINI_KEYS)} key(s) orchestrated",            "telegram_webhook": "✅ active",
        },
    }
