...
class KeyState:
    ...
    def record_error(self, status_code: int, msg: str):
        ...
        self.fail_streak += 1
        if status_code == 429:            cooldown = min(30 * (2 ** self.fail_streak), 600)
...
class SmartKeyOrchestrator:
    ...
    async def report_error(self, key: KeyState, status_code: int, msg: str):
        async with self.lock:            key.record_error(status_code, msg)
...
class GeminiClient:
    ...
    async def generate_response(self, messages: List[Dict]) -> Dict:
        ...
        payload = {
            ...
            "generationConfig": {
                "temperature": 0.80,
                "maxOutputTokens": min(1024, SAFE_TPM - total_input - 200),
...
telegram: Optional[TelegramClient] = Nonehttp_client: Optional[httpx.AsyncClient] = None
RENDER_URL = os.environ.getRENDER_URL", "")
...
async def telegram_webhook(request: Request):
   ...
   _id = str(message.get("from", {}).get("id", ""))
