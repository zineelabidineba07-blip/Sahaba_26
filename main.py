...
# ─── Model Configuration ───
# WARNING: Ensure 'gemini-3-flash-preview' is enabled in your Google AI Studio quota.# Official Docs: https://ai.google.dev/gemini-api/docs/models
...
class SmartKeyOrchestrator:
    def __init__(self, keys: List[str]):
        self.keys: List[KeyState] = [            KeyState(key=k, key_id=f"key_{i}_{uuid.uuid4().hex[:6]}")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "active": active,
            "cooling": cooling,
            "dead": dead,            "total_requests": total_r,
...
class SupabaseClient:
    ...
    async def save_message(self, user_id: str, role: str, content: str, thought_signature: Optional[str] = None):
        try:
...
class GeminiClient:
        ...
        try:
            ...
            for part in parts:
                if "thoughtSignature" in part:
                    return part["thoughtSignature"]        except (KeyError, IndexError):
            pass
        return None
    ...
    async def generate_response(self, messages: List[Dict]) -> Dict:
        ...
        if not ks:            raise HTTPException(503, "No keys available for generation")
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"✅ Key {key_id}… | thinking={thinking_cfg['thinkingConfig']['thinkingLevel']} | tokens={actual_tokens}")
    ...
    async def set_webhook(self, url: str) -> bool:
        try:
            ...
            if data.get("ok"):
                ...
                return True
            logger.error(f"setWebhook failed: {data}")            return False
        except Exception as e:
...
app = FastAPI(
    title="سحابة | Sahaba Bot",    description=f"Telegram bot powered by {MODEL_NAME}",
    version="4.0.0",
    lifespan=lifespan,
)
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        await supabase.save_message(user_id, "assistant", result["reply"], thought_signature=result.get("thought_signature"))
