...
# Source: https://ai.google.dev/gemini-api/docs/models/gemini-3-flash-preview# Input limit: 1,048,576 tokens | Output limit: 65,536 tokens
...
class SmartKeyOrchestrator:
    def __init__(self, keys: List[str]): # Fixed: def __init__
    ...
    async def health_loop(self):        while True:
...
class SupabaseClient:
    def __init__(self, client: httpx.AsyncClient): # Fixed: def __init__
        self.client  = client
        self.headers = {
         "apikey":        SUPABASE_KEY,
         "Authorization": f"Bearer {SUPABASE_KEY}",
         "Content-Type":   "application/json",
         "Prefer":         "return=minimal", # Removed for simplicity
        }
        ...
        try:
            r = await self.client.get(
                ...
                params={
                     ...
                     "limit":   str(limit),
                     "select":   "role,content,thought_signature", # Removed 'thought_signature' if not used elsewhere
                },
                timeout=5.0,
            )
...
# ─────────────────────────────────────────────────────────────
# GEMINI: https://ai.google.dev/gemini-api/docs/models/gemini-3-flash-preview
# ─────────────────────────────────────────────────────────────
class GeminiClient:
    def __init__(self, client: httpx.AsyncClient, orchestrator: SmartKeyOrchestrator): # Fixed: def __init__
        self.client       = client
    ...
    def _build_contents(self, messages: List[Dict]) -> List[Dict]:
        ...
        ─── CRITICAL: Thought Signatures ───────────────────────        Source: https://ai.google.dev/gemini-api/docs/thought-signatures
    ...
    async def generate_response(self, messages: List[Dict]) -> Dict:
        ...
        # ── Step 1: Count tokens accurately ──────────────────        reservation_count = 500
        ...
        # thinking_level is a documented feature for Gemini 3
        # Source: https://ai.google.dev/gemini-api/docs/thinking
        # Using dynamic level based on history length
        history_len = len(messages)
        if history_len < 4:
            thinking_cfg = {"thinkingConfig": {"thinkingLevel": "minimal"}}
        elif history_len < 10:
            thinking_cfg = {"thinkingConfig": {"thinkingLevel": "low"}}
        else:
            thinking_cfg = {"thinkingConfig": {"thinkingLevel": "medium"}}
             ...
             "generationConfig": {
                ...
                # Source: https://ai.google.dev/gemini-api/docs/structured-output
                 ...
                 "responseSchema": {
                     ...
                     "required": ["reply"]                },
                **thinking_cfg, # Apply the dynamic thinking config
            }
        }
...
class TelegramClient:
    ...
    async def send_message(self, chat_id: int, text: str) -> bool:
        ...
        """
        # Removed parse_mode as requested
...
# ─────────────────────────────────────────────────────────────
@app.post("/webhook") # Removed {secret}
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        historyabase.get_history(user_id, limit=20)
...
async def root(request: Request):
    ...
    return {
     ...
     "model":   MODEL_NAME,
     "model_specs": {
     "input_limit":   "1,048,576 tokens",
     "output_limit":  "65,536 tokens",
     "thinking":      "thinking_level (minimal/low/medium/high)",
     "source":        "https://ai.google.dev/gemini-api/docs/models/gemini-3-flash-preview",
    },
     "features": {
     "thinking_level":     "✅ dynamic (minimal→medium based on history)",
     "thought_signatures":  "✅ stored & rehydrated each turn",
     "countTokens_api":    "✅ accurate (includes systemInstruction)",
     "structured_output":  "✅ responseSchema enforced",
     "smart_keys":        f"✅ {len(GEMINI_KEYS)} key(s) orchestrated",
...
if __name__ == "__main__": # Fixed: if __name__
    import uvicorn
